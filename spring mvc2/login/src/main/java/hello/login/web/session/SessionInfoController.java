...
public class SessionInfoController {
    ...
    public String sessionInfo(HttpServletRequest request) {
        ...
        HttpSession session = request.getSession(false);   //세션 불러오기 (새로운 세션 생성 x)
        ...
        session.getAttributeNames().asIterator()
                .forEachRemaining(name -> log.info("session name={}, value={}", name, session.getAttribute(name)));   //배열을 name이라는 이름으로 하나씩 불러와서~
